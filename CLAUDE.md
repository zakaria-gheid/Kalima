...
- **Skip cost:** skipping burns time off the clock (no points) — **configurable in Settings** as a % of the round length (default 10%) or fixed seconds: the timer flashes red, a buzz plays, and a "−Ns" animation flies off the ring. If the time left is ≤ the cost, the skip cannot be paid — the round just ends.
- **Hints:** every word row stores a describer hint in **both languages** (`words.hint_en` / `words.hint_ar`). **Word-specific hints** (e.g. Ear → "on the side of the head, used for hearing") live in `assets/data/word_hints.txt` (`english|hint_en|hint_ar`) and override the per-category fallback hints; extend that file to cover more words and bump `WORD_HINTS_VERSION` in `database.ts`. In-game the describer taps "Hint" to show it (language + time cost configurable in Settings, default Arabic / 5s); the cost burns off the clock like a skip and can't be paid if not enough time remains.
- **Leaderboard stats:** three rankings — teams, best describers, best guessers — each ranked by **points per game** (ties → more games, then total) so different game counts aren't mixed unfairly. A finished game can be **discarded** from the completion screen so it never counts.
