...
3. A **countdown timer** (MM:SS) starts from the chosen round length when the game starts, rendered as a **prominent circular ring** that depletes with time. It supports **pause / resume**; the card is hidden while paused. The last 10 seconds pulse red. A **clock tick** plays each second (toggleable), and when the countdown reaches zero an **alarm + long vibration** fires (toggleable).
4. For each card the group plays the word; the holder marks it **Got it!** (completed — plays a chime) or **Skip**. A live counter of words got is always visible. **Skipping costs time, not points**: 10% of the round length burns off the clock (shown on the Skip button), the ring flashes red, a buzz plays with a short vibration, and a "−Ns" animation flies off the timer. If the remaining time is ≤ the skip cost, the skip cannot be paid and the round ends instead. Sounds and vibration are individually toggleable in Settings (Clock tick / Sound effects / Vibration / Timer end alert).
